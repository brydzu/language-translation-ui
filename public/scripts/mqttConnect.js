...
// init first client
var client_prefix = 'a:' + mqttCreds.IOT_ORG_ID + ':'
...
function addClient(clientName, language) {
          ...
          clients[clientName].subscribe( watson_channel,
            {
              onSuccess: function () {
                ...
                publish(clientName + " has joined the session",  'iot-2/type' + mqttCreds.IOT_DEVICE_TYPE + 'id/' + mqttCreds.IOT_DEVICE_ID + '/evt/logs/fmt/json', 2, 'serverLogs')
  ...
  <button onclick="console.log('sending ' + document.querySelector('#${clientName}_message').value) ;
      ...
      'iot-2/type' + mqttCreds.IOT_DEVICE_TYPE + 'id/' + mqttCreds.IOT_DEVICE_ID + '/evt/fromClient/fmt/json',
      2,
      '${clientName}'
    )"
  >Submit</button>
...
var options = {
     ...
     onSuccess: function () {
         ...
         var watson_channel = 'iot-2/type' + mqttCreds.IOT_DEVICE_TYPE + 'id/' + mqttCreds.IOT_DEVICE_ID + '/evt/'+  "msgin" + '/fmt/json'
